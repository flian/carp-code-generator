 ...
 * Time: 4:14 PM
 */
public class ServiceImplCodeProcessorImpl implements CodeProcessor {
    /**
     * 获取生成代码的模板
     *
     * @param config 全局配置
    ...
    public Template codeTemplate(JpaConfig config) {
        return Freemarker.template(config.getServiceImplTemplateName());
    }
    ...
    public String codeDestination(JpaConfig config) {
        return config.getOutput() + pathSeparator + config.getServiceImplPackage().replaceAll("\\.", pathSeparator) + pathSeparator;
    }
    ...
    public String fileName(JpaConfig config, EntityDto entity) {
        return entity.getServiceImplFileName();
    }
}
