 ...
 * Time: 4:14 PM
 */
public class ServiceInterfaceCodePorcessorImpl implements CodeProcessor {
    /**
     * 获取生成代码的模板
     *
     * @param config 全局配置
    ...
    public Template codeTemplate(JpaConfig config) {
        return Freemarker.template(config.getServiceTemplateName());
    }
    ...
    public String codeDestination(JpaConfig config) {
        return config.getOutput() + pathSeparator + config.getServicePackage().replaceAll("\\.", pathSeparator) + pathSeparator;
    }
    ...
    public String fileName(JpaConfig config, EntityDto entity) {
        return entity.getServiceFileName();
    }
}
